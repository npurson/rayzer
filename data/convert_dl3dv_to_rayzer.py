...
def convert_scene(scene_info, dst_root):
    ...
    try:
        ...
        return (
            f"OK: {scene_hash} ({len(converted_frames)} frames, {actual_w}x{actual_h})"
        )
...
def main():
    ...
    parser.add_argument(
        "--dst", type=str, required=True, help="Output directory for RayZer format data"
    )
    parser.add_argument(
    ...
    with Pool(args.workers) as pool:
        for i, result in enumerate(
            pool.imap_unordered(convert_fn, scenes, chunksize=32)
        ):
            ...
            if (i + 1) % 500 == 0:
                print(
                    f"Progress: {i+1}/{len(scenes)} (OK={ok_count}, SKIP={skip_count}, ERROR={error_count})"
                )
