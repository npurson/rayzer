...
def export_results(result: edict, out_dir: str, compute_metrics: bool = False):
    ...
    for batch_idx in range(input_data.image.size(0)):
        ...
        if compute_metrics:
            ...
            if (
                hasattr(result, "gt_c2w")
                and result.gt_c2w is not None
                and hasattr(result, "c2w")
            ):
                _save_pose_metrics(
...
def visualize_intermediate_results(out_dir, result, save_all=False):
    ...
    for b in range(input.image.size(0)):
        ...
        if ("video_rendering" in result.keys()) and torch.is_tensor(
            result.video_rendering
        ):
            video_rendering = result.video_rendering[b]  # [v, c, h, w]
            _save_video(
                video_rendering, os.path.join(out_dir, f"render_video_{uid}.mp4")
            )
...
def _save_images(result, batch_idx, out_dir):
    ...
    comparison = (
        torch.cat((result.target.image[batch_idx], result.render[batch_idx]), dim=2)
        .detach()
        .cpu()
    )
...
def _save_pose_metrics(gt_c2w, pred_c2w, out_dir, scene_name):
    ...
    pose_metrics = evaluate_camera_pose_metrics(
        gt_c2w, pred_c2w, thresholds=[5, 15, 30]
    )
    ...
    if pose_metrics:
        ...
        with open(pose_csv_file, "w") as f:
            ...
            for i, subfolder in enumerate(pose_valid_subfolders):
                basename = os.path.basename(subfolder)
                values = [
                    str(metric_values[i]) for metric_values in pose_metrics.values()
                ]
                f.write(f"{basename},{','.join(values)}\n")
            ...
            pose_averages = [
                str(sum(values) / len(values)) for values in pose_metrics.values()
            ]
            f.write(f"average,{','.join(pose_averages)}\n")

        print(f"[Pose] Summary written to {pose_csv_file}")
        pose_avg_str = ", ".join(
            f"{k}: {sum(v)/len(v):.4f}" for k, v in pose_metrics.items()
        )
