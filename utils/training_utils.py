...
def create_optimizer(model, weight_decay, learning_rate, betas):
    ...
    optimized_param_dict = {
        name: param for name, param in all_param_dict.items() if param.requires_grad
    }
    ...
    if dist.get_rank() == 0:
        ...
        print(
            f"Optimizer: AdamW, learning rate: {learning_rate}, weight decay: {weight_decay}, betas: {betas}"
        )
...
def create_lr_scheduler(
    optimizer, param_update_steps, warm_up_steps, scheduler_type="cosine"
):
...
def auto_resume_job(load_path, model, optimizer, lr_scheduler, reset_training_state):
    """
    ...
    print_rank0(
        f"Loaded model from {os.path.abspath(ckpt_path)}, the status is {status}"
    )
