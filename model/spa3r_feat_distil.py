...
class PosePredictor(nn.Module):
    def __init__(self, dim, num_heads=16, mlp_ratio=4, trunk_depth=4,
                 init_values=0.01, target_dim=9):
        super().__init__()
        self.target_dim = target_dim
        self.token_norm = nn.LayerNorm(dim)
        self.trunk = nn.Sequential(*[
            MaskedBlock(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                        init_values=init_values)
            for _ in range(trunk_depth)
        ])
        self.trunk_norm = nn.LayerNorm(dim)
        self.register_buffer("empty_pose_tokens", torch.zeros(1, 1, target_dim))
        self.embed_pose = nn.Linear(target_dim, dim)
        self.poseLN_modulation = nn.Sequential(
            nn.SiLU(), nn.Linear(dim, 3 * dim, bias=True),
        )
        self.adaln_norm = nn.LayerNorm(dim, elementwise_affine=False, eps=1e-6)
        self.pose_branch = Mlp(
            in_features=dim, hidden_features=dim // 2,
            out_features=target_dim, drop=0,
        )

    def forward(self, pose_tokens, num_iterations=4):
        """
        Args:
            pose_tokens: [B, S, C]
        Returns:
            list of pose encodings [B, S, 9] (absT_quaR_FoV)
        """
        ...
        for _ in range(num_iterations):
            if pred_pose_enc is None:
                cond = self.embed_pose(self.empty_pose_tokens.expand(B, S, -1))
            else:
                pred_pose_enc = pred_pose_enc.detach()
                cond = self.embed_pose(pred_pose_enc)
            shift, scale, gate = self.poseLN_modulation(cond).chunk(3, dim=-1)
            mod = gate * (self.adaln_norm(pose_tokens) * (1 + scale) + shift) + pose_tokens
            mod = self.trunk(mod)
            delta = self.pose_branch(self.trunk_norm(mod))
            pred_pose_enc = delta if pred_pose_enc is None else pred_pose_enc + delta
            results.append(activate_pose(
                pred_pose_enc, trans_act="linear", quat_act="linear", fl_act="relu",
            ))
        return results
...
class Spa3R(nn.Module):
    def __init__(self, config):
        ...
        self.pose_predictor = PosePredictor(
            dim=D, num_heads=heads,
            trunk_depth=config.model.get("pose_head_depth", 4),
            init_values=iv,
        )
        ...
        self.feature_head = nn.Sequential(nn.LayerNorm(D), nn.Linear(D, D))
        self.rgb_head = nn.Sequential(
            nn.LayerNorm(D), nn.Linear(D, ps * ps * 3), nn.Sigmoid(),
        )

        # --- loss ---
        self.loss_computer = LossComputer(config)
        self.feat_w = config.training.get("feature_loss_weight", 1.0)
        self.palign_w = config.training.get("pose_align_loss_weight", 1.0)
    ...
    def _forward_train(self, data, create_visual=False):
        ...
        # pose_token_2: 只看 aug_b（更受限），对齐 pose_token_1 的预测结果
        pose_tok2 = torch.cat([cam_ctx_feat.detach(), pose2_feat], 1)  # [B,2,C]
        pose_enc2 = self.pose_predictor(pose_tok2)[-1]             # [B,2,9]
        # 用预测出的 pose encoding 算对齐 loss，而不是直接对 feature 算
        palign_loss = F.smooth_l1_loss(pose_enc2, pose_enc1.detach())
        ...
        plucker = cam_to_plucker(tgt_c2w, tgt_fxfycxcy, H_eff, W_eff)
        plk_tok = self.plucker_tokenizer(plucker)                  # [B,N,C]

        aug_b_idx = (pmask == 1)                                   # [B,N]
        n_b = int(aug_b_idx[0].sum().item())

        plk_b   = torch.stack([plk_tok[b][aug_b_idx[b]] for b in range(B)])
        feat_gt = torch.stack([tgt_patch_f[b][aug_b_idx[b]] for b in range(B)]).detach()
        ...
        dec_in = torch.cat([plk_b, ctx_patch_f], 1)
        dec_in = self.decoder_ln(dec_in)
        for blk in self.decoder_blocks:
            dec_in = torch_checkpoint(blk, dec_in, None, None, use_reentrant=False)
        dec_b = dec_in[:, :n_b]

        pred_feat = self.feature_head(dec_b)
        pred_rgb  = self.rgb_head(dec_b)

        # 11. losses
        feat_sl1  = F.smooth_l1_loss(pred_feat, feat_gt)
        feat_cos  = (1.0 - F.cosine_similarity(pred_feat, feat_gt, dim=-1)).mean()
        feat_loss = 1 * feat_sl1 + 0.5 * feat_cos

        rgb_loss  = F.mse_loss(pred_rgb, rgb_gt)
        psnr      = -10.0 * torch.log10(rgb_loss.clamp(min=1e-8))

        loss = rgb_loss + self.feat_w * feat_loss + self.palign_w * palign_loss

        ret = edict(
            loss_metrics=edict(
                loss=loss, rgb_l2_loss=rgb_loss, psnr=psnr,
                feature_loss=feat_loss,
                feat_smooth_l1=feat_sl1, feat_cosine=feat_cos,
                pose_align_loss=palign_loss,
            ),
        ...
        if create_visual:
            # Reconstruct predicted image: GT patches for aug_a, predicted for aug_b
            pred_patches = tgt_rgb_patches.clone()
            for b_i in range(B):
                pred_patches[b_i][aug_b_idx[b_i]] = pred_rgb[b_i].detach().to(pred_patches.dtype)
            pred_img = rearrange(
                pred_patches,
            ...
            ret.input  = edict(
                image=ctx_img_eff.unsqueeze(1).detach().clone(),   # [B, 1, 3, H_eff, W_eff]
                index=data["index"][:, 0:1],                       # [B, 1, 2]
                scene_name=data.get("scene_name", [""]*B),
            )
            ret.target = edict(
                image=tgt_img_eff.unsqueeze(1).detach().clone(),   # [B, 1, 3, H_eff, W_eff]
                index=data["index"][:, 1:2],                       # [B, 1, 2]
                scene_name=data.get("scene_name", [""]*B),
            )
            ret.render = pred_img.unsqueeze(1).detach()            # [B, 1, 3, H_eff, W_eff]
    ...
    def _forward_inference(self, data, create_visual=False):
        ...
        for v in range(num_in):
            ctx_feats.append(ftoks[v][:, self.patch_start_idx:])
        ctx_f = torch.cat(ctx_feats, 1)                           # [B, num_in*N, C]
    ...
    def get_overview(self):
        ...
        return edict(
            ...
            feature_head=c(self.feature_head),
