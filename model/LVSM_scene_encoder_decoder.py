...
class Images2LatentScene(nn.Module):
    ...
    def get_posed_input(
        self, images=None, ray_o=None, ray_d=None, method="default_plucker"
    ):
        """
        Args:
            images: [b, v, c, h, w]
    ...
    def forward(self, data_batch, has_target_image=True):
        ...
        input_img_tokens = self.image_tokenizer(
            posed_input_images
        )  # [b*v, n_patches, d]

        _, n_patches, d = input_img_tokens.size()  # [b*v, n_patches, d]
        input_img_tokens = input_img_tokens.reshape(
            b, v_input * n_patches, d
        )  # [b, v*n_patches, d]

        latent_vector_tokens = self.n_light_field_latent.expand(
            b, -1, -1
        )  # [b, n_latent_vectors, d]

        encoder_input_tokens = torch.cat(
            (latent_vector_tokens, input_img_tokens), dim=1
        )  # [b, n_latent_vectors + v*n_patches, d]
        ...
        repeated_latent_tokens = repeat(
            latent_tokens, "b nl d -> (b v_target) nl d", v_target=v_target
        )

        target_pose_tokens = self.target_pose_tokenizer(
            target_pose_cond
        )  # [b*v_target, n_patches, d]

        decoder_input_tokens = torch.cat(
            (target_pose_tokens, repeated_latent_tokens), dim=1
        )  # [b*v_target, n_latent_vectors + n_patches, d]
        decoder_input_tokens = self.transformer_input_layernorm_decoder(
    ...
    def render_video(
        ...
        intermediate_tokens = self.pass_layers(
            self.transformer_encoder, encoder_input_tokens, gradient_checkpoint=False
        )
        ...
        if traj_type == "interpolate":
            ...
            homogeneous_row = torch.tensor([[[0, 0, 0, 1]]], device=device).expand(
        ...
        target_pose_tokens = self.target_pose_tokenizer(
            target_pose_cond
        )  # [bs*v_target, n_patches, d]
        _, n_patches, d = target_pose_tokens.size()  # [b*v_target, n_patches, d]
        target_pose_tokens = target_pose_tokens.reshape(
            bs, num_views * n_patches, d
        )  # [b, v_target*n_patches, d]
        ...
        for cur_chunk in range(0, num_views, view_chunk_size):
            ...
            start_idx, end_idx = (
                cur_chunk * n_patches,
                (cur_chunk + cur_view_chunk_size) * n_patches,
            )
            ...
            cur_repeated_latent_tokens = repeat(
                latent_tokens, "b nl d -> (b v_chunk) nl d", v_chunk=cur_view_chunk_size
            )

            decoder_input_tokens = torch.cat(
                (cur_target_pose_tokens, cur_repeated_latent_tokens), dim=1
            )
            decoder_input_tokens = self.transformer_input_layernorm_decoder(
                decoder_input_tokens
            )
